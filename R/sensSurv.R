...
#'
Survsens = function(data = toy,object_name = match1,exp,outcome,failtime,Gamma,alpha,Gammainterval,plot_title = NULL){
  results = list()
  ...
  plot= ggplot(data = table1, aes(x = gamVal,y = pupper))+geom_line()+geom_point(aes(x=vrt,y=hrz))+ylab("p upper bound")+xlab("gamma (Bias)")+theme_bw()+annotate("text",x=vrt+0.1*vrt,y = hrz,label=paste0("(",vrt1,",",hrz1,")"))+labs(title = plot_title, caption = paste("matching done by",class(object_name),"function"))+theme(plot.title = element_text(hjust = 0.5))
