...
rubinRules = function(data, Treatment, matchscore = "ps", covlist) {
    ...
    results$RUBIN1 <- with(data1a, abs(100 * (mean(matchscore[Treatment == "1"]) - mean(matchscore[Treatment ==
        "0"]))/sd(matchscore)))

    # Rubin 2

    results$RUBIN2 <- with(data1a, var(matchscore[Treatment == "1"])/var(matchscore[Treatment ==
        "0"]))
    ...
    dotchart(d, pch = 15, col = "black", main = "Rubin's Rules plot", xlab = "Residual Variance Ratio",
        xlim = c(low, high))
    abline(v = 1, lty = 1)
...
ampPlot <- function(gamma, lambda) {
    ...
    ggplot(sensdata, aes(x = lambda, y = delta)) + geom_line() + theme_bw() + annotate("text",
        x = sensdata[1, 1], y = sensdata[1, 2], hjust = -0.5, label = paste0("(", sensdata[1,
            1], ",", sensdata[1, 2], ")")) + annotate("text", x = sensdata[2, 1], y = sensdata[2,
        2], hjust = -0.5, label = paste0("(", sensdata[2, 1], ",", sensdata[2, 2], ")")) + annotate("text",
        x = sensdata[3, 1], y = sensdata[3, 2], hjust = 1.2, label = paste0("(", sensdata[3,
            1], ",", sensdata[3, 2], ")")) + geom_point()
}
...
binSensgraph = function(x, y = NULL, Gamma = 3, GammaInc = 0.2, alpha = 0.06) {
    if (length(x) == 1) {
        ...
        # in case you are passing an object from another function
    ...
    k = ggplot(data = bounds, aes(x = gamma, y = pupper)) + geom_line() + geom_point(aes(x = vrt,
        y = hrz)) + ylab("p upper bound") + xlab("gamma (Bias)") + theme_bw() + annotate("text",
        x = vrt1 + 0.1, y = hrz1, label = paste0("(", vrt1, ",", hrz1, ")")) + labs(title = "Binary Outcome Sensitivity Plot")

    # plot(bounds$pupper ~ bounds$gamma, type = 'l', xlab = 'Gamma', ylab = 'p-val upper bound',
    # main = 'Sensitivity plot for binary outcomes')
    # text(vrt,hrz,paste0('(',vrt,',',hrz,')'),pos = 2) points(vrt,hrz,pch=15)
...
sensbin <- function(data, match_f, object_name, exposure, outcome, Gamma, GammaInc, alpha = 0.05) {
    ...
    plot = ggplot(data = bounds, aes(x = gamma, y = pupper)) + geom_line() + geom_point(aes(x = vrt,
        y = hrz)) + ylab("p upper bound") + xlab("gamma (Bias)") + theme_bw() + annotate("text",
        x = vrt1 + 0.1, y = hrz1, label = paste0("(", vrt1, ",", hrz1, ")")) + labs(title = "Binary Outcome Sensitivity Plot")
...
#'
Survsens = function(data = toy, object_name = match1, exp, outcome, failtime, Gamma, alpha,
    ...
    plot = ggplot(data = table1, aes(x = gamVal, y = pupper)) + geom_line() + geom_point(aes(x = vrt,
        ...
        caption = paste("matching done by", class(object_name), "function")) + theme(plot.title = element_text(hjust = 0.5))
