...
```{r rubinRules,fig.width = 7.5, fig.asp =1,message=FALSE,warning=FALSE}
...
```{r modifyrubinRules,fig.width = 7.5, fig.asp =1,message=FALSE,warning=FALSE}
...
This example uses to design match which implements cardinality matching which balances variables directly rather than using a summary measure to quantify the distance between observations. This matching optimizes on the sample size. To implement cardinality matchng (using the lalonde data), the following code can be be executed
...
To assesse balance with `love_plot`function in this package you would need to execute the following code

```{r LoveplotDesignMatch}
p = love_plot(X =out, data = lalonde , covList=c("age", "educ", "black", "hispan", "married", "nodegree", "re74", "re75"))

p
```

This plot can be enhanced using `ggplot2` features. For example we can add disired standardized difference treshholds of , say , -0.1 to 0.1 

```{r modifyLoveplotDesignMatch}
p+theme_bw()+geom_vline(xintercept = -0.1)+ geom_vline(xintercept = 0.1)

```

### Using `MatchIt` or `Matching` objects
