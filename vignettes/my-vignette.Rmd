...
library(tidyr);library(ggplot2);library(designmatch);library(MatchIt);library(Matching);library(sensitivitymv);library(sensitivitymw);library(knitr);library(cobalt);library(designmatch)
```
...
```{r cardinalityMatch,echo=FALSE,warning=FALSE,message=FALSE,eval=FALSE}
...
```{r LoveplotDesignMatch,fig.width=6,fig.asp=1,message=FALSE,warning=FALSE,eval=FALSE}
...
```{r modifyLoveplotDesignMatch,fig.width=6,fig.asp=1,warning=FALSE,message=FALSE,eval=FALSE}
...
```{r designCont,message=FALSE,warning=FALSE}
...
covs0 = c("age", "educ", "black", "hispan", "married", "nodegree", "re74", "re75")
...
This function returns Rosenbaum sensitivity bounds and a rearranged dataset. The first few rows of the data returned by the function are shown below
