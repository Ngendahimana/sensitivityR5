...
k =rubinRules(data=toy,Treatment='treated',covlist=covlist1)
k$plot

```

The first two Rubin balance meausures are displayed at the top of the plot while the third measures which consists of series of variance ratios for each variable are displayed inside the dot plot. To obtain the actual numerical values , various items can be called off of the list (k) using conventional R methods. For example to obtain the numerical values for the thired rubin guideline the following line of code can be executed:
